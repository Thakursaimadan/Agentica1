...
userRouter.post("/process-mess-requests", async (req, res) => {
	try {
		...
		if (requests.rowCount === 0)
			return res.json({ message: "No requests to process" });
		let countAtoB = 0,
			countBtoA = 0;
		const requestMap = {};
	  ...
	  `);
		...
		for (let req of requests.rows) {
			...
			if (mess.current_count < mess.capacity) {
				...
				// Update mess counts
				await pool.query(
					`UPDATE messes SET current_count = current_count + 1 WHERE mess_id = $1`,
					[requested_mess_id]
				);
				await pool.query(
					`UPDATE messes SET current_count = current_count - 1 WHERE mess_id = (SELECT mess_id FROM users WHERE user_id = $1)`,
					[user_id]
				);
